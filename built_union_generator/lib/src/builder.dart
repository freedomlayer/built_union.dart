...
/// _$SimpleUnion.integer(int integer): _type = _$SimpleUnionType.integer, _values = [integer];
/// ```
String generateClassConstructor(String className, String enumName, VariantSpec variant) {
  List<String> res = [];
...
String generateClassConstructors(UnionSpec unionSpec) {
  ...
  for (final variant in unionSpec.variants) {
    res.add(generateClassConstructor(calcClassName(unionSpec), calcEnumName(unionSpec), variant));
  }
  return res.join('\n');
}
